...
for preselection in np.array([1, 0]):
    ...
    if preselection == 0:
        D_training_data_raw = np.loadtxt(
            "D_train" + string_V_max + "_preselection.0.dat"
        )
        D_validating_data_raw = np.loadtxt(
            "D_validate" + string_V_max + "_preselection.0.dat"
        )
    ...
    if preselection == 1:
        D_training_data_raw = np.loadtxt(
            "D_train" + string_V_max + "_preselection.1.dat"
        )
        D_validating_data_raw = np.loadtxt(
            "D_validate" + string_V_max + "_preselection.1.dat"
        )
    ...
    D_y_validating_raw_integers = np.abs(
        np.rint(D_validating_data_raw[:, index_D_C_bulk])
    )
    D_y_testing_raw_integers = np.abs(np.rint(D_testing_data_raw[:, index_S_C_bulk]))

    D_y_training_categorical = keras.utils.np_utils.to_categorical(
        D_y_training_raw_integers, num_classes
    )
    D_y_validating_categorical = keras.utils.np_utils.to_categorical(
        D_y_validating_raw_integers, num_classes
    )
    D_y_testing_categorical = keras.utils.np_utils.to_categorical(
        ...
        for run_i in range(0, number_of_runs):
            for NN_architecture in NN_architecture_vec:
                ...
                if NN_architecture == 0:
                    ...
                    filepath = (
                        "model_keras" + parameters_NN + "_epochs.{epoch:03d}.keras"
                    )
                    ...
                    print(
                        "NN duration fit: ",
                        "{:2.2f}".format(stop_time - start_time),
                        "[s]",
                    )
                ...
                if NN_architecture == 1:
                    ...
                    with open(
                        "model_keras_summary" + parameters_NN + "_report.txt", "w"
                    ) as fh:
                    ...
                    history = model.fit(
                        D_X_training_raw.reshape(D_X_training_raw.shape[0], Nx, Ny, 1),
                        D_y_training_categorical,
                        shuffle="True",
                        batch_size=Batch_size,
                        epochs=Number_of_epochs,
                        verbose=1,
                        validation_data=(
                            D_X_validating_raw.reshape(
                                D_X_validating_raw.shape[0], Nx, Ny, 1
                            ),
                            D_y_validating_categorical,
                        ),
                        callbacks=[checkpoint],
                ...
                np.savetxt(
                    "accuracy_keras_vs_epochs_training_D"
                    + parameters_NN
                    + ".callback_history",
                    np.column_stack(
                        (
                            np.arange(1, Number_of_epochs + 1),
                ...
                np.savetxt(
                    "loss_keras_vs_epochs_training_D"
                    + parameters_NN
                    + ".callback_history",
                    np.column_stack(
                        (
                ...
                np.savetxt(
                    "loss_keras_vs_epochs_validating_D"
                    + parameters_NN
                    + ".callback_history",
                    np.column_stack(
                        (
