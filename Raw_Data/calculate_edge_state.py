...
(
    ...
    "kx_s ky_s k2_s k4_s c_s M g1_s g2_s parameters_s eta_s delta_s m_s phi_s s_s",
    real=True,
)
...
#%%
row_1 = [
    m_s * exp(I * phi_s) + cos(kx_s) + cos(ky_s),
    s_s * (sin(kx_s) - I * sin(ky_s)),
]
row_2 = [
    s_s * (sin(kx_s) + I * sin(ky_s)),
    -m_s * exp(I * phi_s) - cos(kx_s) - cos(ky_s),
...
#%%
def H_n_fourier_component_of_k1(
    n, k2, parameters
    ...
    if n > 0:
        ...
        for l in range(l_min, l_max + 1):
            k_l = dk * l
            h_n_mode = h_n_mode + np.exp(-1j * k_l * n) * get_H_in_k1_k2(
                k_l, k2, parameters
            )
...
#%%
def get_H_N1_k2(
    N1, k2, hopping_range, parameters
    ...
    H_on_diagonal = np.kron(
        np.eye(N1, k=0), H_n_fourier_component_of_k1(n, k2, parameters)
    )
    for n in np.arange(1, hopping_range + 1):
        H_off_diagonal = H_off_diagonal + np.kron(
            np.eye(N1, k=n), H_n_fourier_component_of_k1(n, k2, parameters)
        )
        ...
        for k2 in k2_vec:
            ...
            for ii in range(Eigenenergies.shape[0]):
                ...
                stringA = (
                    ...
                    + str(ii)
                    + " "
                    + "{:2.4f}".format(Eigenenergies[ii].real)
                    + "\n"
                )
                #                Psi = P[:,ii]
                #
