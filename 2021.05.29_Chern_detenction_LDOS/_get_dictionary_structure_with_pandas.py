...
#%%
data_train_preselection_0, data_validate_preselection_0, data_test_preselection_0 = train_validate_test_split_data_raw(data_raw_V_max_1, train_percent=.8, validate_percent=.1, seed=None)
...
idx = np.where( np.abs( np.abs(data_raw_V_max_1[:,index_C_bulk]) - np.abs(data_raw_V_max_1[:,index_C_marker_disorder]) ) < 0.5)[0]    
data_raw_V_max_1_preselected = data_raw_V_max_1[idx,:]

 
data_train_preselection_1, data_validate_preselection_1, data_test_preselection_1 = train_validate_test_split_data_raw(data_raw_V_max_1_preselected, train_percent=.8, validate_percent=.1, seed=None)
...
#%%
data_train_preselection_0, data_validate_preselection_0, data_test_preselection_0 = train_validate_test_split_data_raw(data_raw_V_max_2, train_percent=.8, validate_percent=.1, seed=None)
...
idx = np.where( np.abs( np.abs(data_raw_V_max_2[:,index_C_bulk]) - np.abs(data_raw_V_max_2[:,index_C_marker_disorder]) ) < 0.5)[0]    
data_raw_V_max_2_preselected = data_raw_V_max_2[idx,:]

 
data_train_preselection_2, data_validate_preselection_2, data_test_preselection_2 = train_validate_test_split_data_raw(data_raw_V_max_2_preselected, train_percent=.8, validate_percent=.1, seed=None)
...
dataFrame_train, dataFrame_validate, dataFrame_test = train_validate_test_split(dataFrame, train_percent=.6, validate_percent=.2, seed=None)
dataFrame_train.reset_index(inplace=True)
dataFrame_test.reset_index(inplace=True)
dataFrame_validate.reset_index(inplace=True)
...
bins = 100
condition_0 = np.logical_and(np.round(dataFrame_tmp['Chern_bulk'])==0 , dataFrame_tmp['v_sample'] < 1)  
condition_1 = np.logical_and(np.round(dataFrame_tmp['Chern_bulk'])==1 , dataFrame_tmp['v_sample'] < 1)  
condition_2 = np.logical_and(np.round(dataFrame_tmp['Chern_bulk'])==2 , dataFrame_tmp['v_sample'] < 1)  
condition_3 = np.logical_and(np.round(dataFrame_tmp['Chern_bulk'])==3 , dataFrame_tmp['v_sample'] < 1)  
...
bins = 100
condition_0 = np.logical_and(np.round(dataFrame_tmp['Chern_bulk'])==0 , dataFrame_tmp['v_sample'] < 1)  
condition_1 = np.logical_and(np.round(np.round(dataFrame_tmp['Chern_bulk'])==1) , dataFrame_tmp['v_sample'] < 1)  
condition_2 = np.logical_and(np.round(np.round(dataFrame_tmp['Chern_bulk'])==2) , dataFrame_tmp['v_sample'] < 1)  
condition_3 = np.logical_and(np.round(np.round(dataFrame_tmp['Chern_bulk'])==3) , dataFrame_tmp['v_sample'] < 1)  
idx_C_bulk_0 = np.where(condition_0)[0]
idx_C_bulk_1 = np.where(condition_1)[0]
idx_C_bulk_2 = np.where(condition_2)[0]
idx_C_bulk_3 = np.where(condition_3)[0]
